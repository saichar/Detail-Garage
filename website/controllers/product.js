...
class ProductController{
    ...
    getCategoryProductList(req, res){   
        ...
        if(req.body.user_id != ''){
            ...
            var sql = "SELECT tp.* , IFNULL((SELECT IFNULL(id, 0) AS favorite from tbl_wishlist_product twp WHERE twp.product_id = tp.id AND twp.user_id = "+req.body.user_id+" ), 0) AS favorite FROM tbl_products tp INNER JOIN tbl_categories AS tc ON (tc.id = tp.category_id) WHERE tc.slug = (?) AND tp.status = 1 AND tp.is_deleted = 0 GROUP BY tp.id LIMIT " + offset + "," + newlimit + "";
        } else {
            var sql = "SELECT tp.* FROM tbl_products tp INNER JOIN tbl_categories AS tc ON (tc.id = tp.category_id) WHERE tc.slug = (?) AND tp.status = 1 AND tp.is_deleted = 0 GROUP BY tp.id LIMIT " + offset + "," + newlimit + "";
    ...
    getFilterCategoryProductList(req, res){
        ...
        if(req.body.user_id != ''){
            ...
            var sql = "SELECT tp.* , IFNULL((SELECT IFNULL(id, 0) AS favorite from tbl_wishlist_product twp WHERE twp.product_id = tp.id AND twp.user_id = "+req.body.user_id+" ), 0) AS favorite FROM tbl_products tp INNER JOIN tbl_categories AS tc ON (tc.id = tp.category_id) WHERE tc.slug = '"+req.body.slug+"' AND tp.status = 1 AND tp.is_deleted = 0"+Where1+Where2+orderBy;
        } else {
            ...
            var sql = "SELECT tp.* FROM tbl_products tp INNER JOIN tbl_categories AS tc ON (tc.id = tp.category_id) WHERE tc.slug = '"+req.body.slug+"' AND tp.status = 1 AND tp.is_deleted = 0"+Where1+Where2+orderBy;
        }
    ...
    productCheckout(req, res){
        ...
        let sql = "INSERT INTO tbl_order (user_id, transaction_no, stripe_txn_id, stripe_id, amount, total_amount, shipping_amount, shipping_type, address_id, addedOn) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        conn.query(sql, [req.body.user_id, transactionId, req.body.stripeTxnId, req.body.stripeId, req.body.amount, req.body.total_amount, req.body.shipping_amount, req.body.shipping_type, req.body.address_id, addedOn], function (err, rows) {
            if(err){
                res.json({status: "false", response: rows, message: 'Something went wrong!'});
                res.end();
            }else{
    ...
    searchProductList_OLD(req, res){
		...
		async.parallel({
            ...
            category: function (calb) {
                ...
                var sql = "SELECT sct.id as succat From tbl_categories ct INNER JOIN tbl_categories sct ON ct.id = sct.parent_id WHERE  ct.category_name LIKE '%"+name+"%' OR  ct.slug LIKE '%"+name+"%' OR  sct.category_name LIKE '%"+name+"%'";
    ...
    async.parallel({
        ...
        if(results.productlist.length>0 && results.category.length>0){
            ...
            if(req.body.user_id != ''){
                ...
                var sql = "SELECT * FROM tbl_products WHERE status = 1 AND is_deleted = 0 AND sub_category_id IN ("+newarrcat.toString()+") OR id IN ("+newarr.toString()+") GROUP BY id LIMIT " + offset + "," + newlimit + "";
            }
            ...
            if(req.body.user_id != ''){
                ...
                var sql = "SELECT * FROM tbl_products WHERE status = 1 AND is_deleted = 0 AND sub_category_id IN ("+newarr.toString()+") GROUP BY id LIMIT " + offset + "," + newlimit + "";
            }
            ...
            if(req.body.user_id != ''){
                ...
                var sql = "SELECT * FROM tbl_products WHERE status = 1 AND is_deleted = 0 AND id IN ("+newarr.toString()+") GROUP BY id LIMIT " + offset + "," + newlimit + "";
            }
        }
...
getFilterSearchProductList(req, res){ 
    var name=req.body.name;
    var sortBy=req.body.sortby;
    var subCategoryId =req.body.subCategoryId;
    var min_price =req.body.min_price;
    ...
    async.parallel({
        ...
        if(results.productlist.length>0 && results.category.length>0){
            ...
            if(req.body.user_id != ''){
                ...
                var sql = "SELECT tp.* , IFNULL((SELECT IFNULL(id, 0) AS favorite from tbl_wishlist_product twp WHERE twp.product_id = tp.id AND twp.user_id = "+req.body.user_id+" ), 0) AS favorite FROM tbl_products tp LEFT JOIN tbl_wishlist_product twp ON tp.id = twp.product_id WHERE tp.status = 1 AND tp.is_deleted = 0 AND (tp.sub_category_id IN ("+newarrcat.toString()+") OR tp.id IN ("+newarr.toString()+")) "+Where1+Where2+orderBy+" LIMIT " + offset + "," + newlimit + "";

                
                
            } else {
                var sql = "SELECT * FROM tbl_products WHERE status = 1 AND is_deleted = 0 AND (sub_category_id IN ("+newarrcat.toString()+") OR id IN ("+newarr.toString()+")) "+Where1+Where2+orderBy+" LIMIT " + offset + "," + newlimit + "";
            }
            ...
            if(req.body.user_id != ''){
                var sql = "SELECT tp.* , IFNULL((SELECT IFNULL(id, 0) AS favorite from tbl_wishlist_product twp WHERE twp.product_id = tp.id AND twp.user_id = "+req.body.user_id+" ), 0) AS favorite FROM tbl_products tp LEFT JOIN tbl_wishlist_product twp ON tp.id = twp.product_id WHERE tp.status = 1 AND tp.is_deleted = 0 AND (tp.sub_category_id IN ("+newarr.toString()+")) "+Where1+Where2+orderBy+" LIMIT " + offset + "," + newlimit + "";
            } else {
                var sql = "SELECT * FROM tbl_products WHERE status = 1 AND is_deleted = 0 AND sub_category_id IN ("+newarr.toString()+") "+Where1+Where2+orderBy+" LIMIT " + offset + "," + newlimit + "";
            }
            ...
            if(req.body.user_id != ''){
                var sql = "SELECT tp.* , IFNULL((SELECT IFNULL(id, 0) AS favorite from tbl_wishlist_product twp WHERE twp.product_id = tp.id AND twp.user_id = "+req.body.user_id+" ), 0) AS favorite FROM tbl_products tp LEFT JOIN tbl_wishlist_product twp ON tp.id = twp.product_id WHERE tp.status = 1 AND tp.is_deleted = 0 AND tp.id IN ("+newarr.toString()+") "+Where1+Where2+orderBy+" LIMIT " + offset + "," + newlimit + "";
            } else {
                var sql = "SELECT * FROM tbl_products WHERE status = 1 AND is_deleted = 0 AND id IN ("+newarr.toString()+") "+Where1+Where2+orderBy+" LIMIT " + offset + "," + newlimit + "";
            }
        } else { console.log("ELSE");
            if(req.body.user_id != ''){
                var sql = "SELECT tp.* , IFNULL((SELECT IFNULL(id, 0) AS favorite from tbl_wishlist_product twp WHERE twp.product_id = tp.id AND twp.user_id = "+req.body.user_id+" ), 0) AS favorite FROM tbl_products tp LEFT JOIN tbl_wishlist_product twp ON tp.id = twp.product_id WHERE tp.status = 1 AND tp.is_deleted = 0"+Where1+Where2+orderBy+" LIMIT " + offset + "," + newlimit + "";
            } else {
                var sql = "SELECT * FROM tbl_products WHERE status = 1 AND is_deleted = 0"+Where1+Where2+orderBy+" LIMIT " + offset + "," + newlimit + "";
            }
        }
        // console.log(sql);
    ...
    searchProductListbyprice(req, res){
		...
		async.parallel({
            ...
            category: function (calb) {
                ...
                var sql = "SELECT sct.id as succat From tbl_categories ct LEFT JOIN tbl_categories sct ON ct.id = sct.parent_id WHERE  ct.category_name LIKE '%"+name+"%' OR  ct.slug LIKE '%"+name+"%' OR  sct.category_name LIKE '%"+name+"%'";
